...
do-profile-check: $(depsdir)/.stamp
	...
	ok=false; \
	if $$ok; then rm -f $(PROFILE_EXE) $(PROFILE_OUT); else \
		$(MAKE) -C ../ get-monolite-latest ; \
	    if test -f $(MONOLITE_MSCORLIB); then \
		$(MAKE) V=1 do-profile-check-monolite ; \
	    else \
		echo "*** The runtime '$(PROFILE_RUNTIME)' doesn't appear to be usable." 1>&2; \
                echo "*** You need Mono version 4.2 or better installed to build MCS" 1>&2 ; \
...
$(PROFILE_EXE): $(topdir)/build/common/basic-profile-check.cs
		ls -l $(topdir)/../external/roslyn-binaries/Microsoft.Net.Compilers/Microsoft.Net.Compilers.1.3.2/tools/
