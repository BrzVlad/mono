...
interp_entry_from_trampoline (CallContext *ccontext, InterpMethod *rmethod)
{
	...
	for (i = 0; i < sig->param_count + sig->hasthis; i++) {
		MonoType *type = sig->params [i - sig->hasthis];
		if (type->type == MONO_TYPE_GENERICINST && !MONO_TYPE_IS_REFERENCE (type)) {
			frame.stack_args [i].data.vt = alloca (mono_class_value_size (type->data.klass, NULL));
		} else if (type->type == MONO_TYPE_VALUETYPE) {
			if (sig->pinvoke)
				frame.stack_args [i].data.vt = alloca (mono_class_native_size (type->data.klass, NULL));
			else
				frame.stack_args [i].data.vt = alloca (mono_class_value_size (type->data.klass, NULL));
		}
	}
