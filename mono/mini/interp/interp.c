...
ves_imethod (InterpFrame *frame, MonoMethod *method, MonoMethodSignature *sig, stackval *ret_sp, stackval *sp)
{
	const char *name = method->name;
	...
	if (method->klass == mono_defaults.array_class) {
		if (!strcmp (name, "UnsafeMov")) {
			/* TODO: layout checks */
			stackval_from_data (sig->ret, ret_sp, (char*) sp, FALSE);
			return NULL;
		}
		if (!strcmp (name, "UnsafeLoad"))
			return ves_array_get (frame, sp, ret_sp, sig, FALSE);
	}
...
#ifndef ENABLE_NETCORE
			MonoMethod *target_method = (MonoMethod*) frame->imethod->data_items [ip [3]];
			MonoMethodSignature *sig = (MonoMethodSignature*) frame->imethod->data_items [ip [4]];

			MonoException *ex = ves_imethod (frame, target_method, sig, (stackval*)(locals + ip [1]), (stackval*)(locals + ip [2]));
			if (ex)
				THROW_EX (ex, ip);
...
#define SHIFTOP(datatype, op) \
	...
	ip += 4;
		...
		MINT_IN_CASE(MINT_NEWOBJ_ARRAY) {
			...
			LOCAL_VAR (ip [1], MonoObject*) = ves_array_create (frame->imethod->domain, newobj_class, param_count, (stackval*)(locals + ip [2]), error);
			if (!is_ok (error))
				THROW_EX (mono_error_convert_to_exception (error), ip);
			ip += 5;
			MINT_IN_BREAK;
	...
	*(fieldtype*)addr = LOCAL_VAR (ip [1], datatype); \
	ip += 4; \
		...
		MINT_IN_CASE(MINT_LDELEMA_TC) {
			...
			MonoException *ex = ves_array_element_address (frame, klass, (MonoArray *) o, locals + ip [1], sp + 1, TRUE);
			if (ex)
				THROW_EX (ex, ip);
			ip += 4;
			MINT_IN_BREAK;
