...
emit_local_symbol (MonoAotCompile *acfg, const char *name, const char *end_label, gboolean func)
{
	...
	if (NULL != mangled_symbol_name_alloc) {
		g_free_vb (mangled_symbol_name_alloc);
	}
}
...
arch_emit_imt_trampoline (MonoAotCompile *acfg, int offset, int *tramp_size)
{
	...
	code = buf = (guint8 *)g_malloc_vb (256);
...
emit_offset_table (MonoAotCompile *acfg, const char *symbol, MonoAotFileTable table, int noffsets, int group_size, gint32 *offsets)
{
	...
	buf_size = noffsets * 4;
	p = buf = (guint8 *)g_malloc0_vb (buf_size);
		...
		if (cattr) {
			...
			if (j < cattr->num_attrs) {
				...
				if (num_named == 1) {
					...
					export_name = (char *)g_malloc_vb (slen + 1);
...
emit_method_info (MonoAotCompile *acfg, MonoCompile *cfg)
{
	...
	p = buf = (guint8 *)g_malloc_vb (buf_size);
...
emit_trampoline_full (MonoAotCompile *acfg, int got_offset, MonoTrampInfo *info, gboolean emit_tinfo)
{
	...
	buf = (guint8 *)g_malloc_vb (buf_size);
	p = buf;
...
add_readonly_value (MonoAotOptions *opts, const char *val)
{
	...
	rdv->name = (char *)g_malloc0_vb (tval - val + 1);
...
mono_aot_method_hash (MonoMethod *method)
{
	...
	hashes_start = (guint32 *)g_malloc0_vb (hashes_count * sizeof (guint32));
	hashes = hashes_start;
...
emit_exception_info (MonoAotCompile *acfg)
{
	...
	if (seq_points_to_file) {
		...
		g_free_vb (aotid);
		g_free_vb (dir);
		g_free_vb (image_basename);
		g_free_vb (aot_file);
		g_free_vb (aot_file_path);
	}
...
emit_image_table (MonoAotCompile *acfg)
{
	...
	buf = p = (guint8 *)g_malloc0_vb (buf_size);
...
#define LD_OPTIONS ""
	...
	if (execute_system (command) != 0) {
		g_free_vb (tmp_outfile_name);
		g_free_vb (outfile_name);
		g_free_vb (command);
		g_free_vb (objfile);
		g_free_vb (ld_flags);
		return 1;
...
acfg_create (MonoAssembly *ass, guint32 opts)
{
	...
	acfg->mempool = mono_mempool_new_runtime (G_STRLOC, 0);
...
acfg_free_vb (MonoAotCompile *acfg)
{
	...
	g_free_vb (acfg->static_linking_symbol);
	g_free_vb (acfg->got_symbol);
	g_free_vb (acfg->plt_symbol);
	...
	g_free_vb (acfg->patch_to_plt_entry);
