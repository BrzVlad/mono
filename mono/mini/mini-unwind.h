...
#define mono_add_unwind_op_register(op_list,code,buf,reg,reg_loc) do { (op_list) = g_slist_append ((op_list), mono_create_unwind_op ((code) - (buf), DW_CFA_register, (reg), (reg_loc))); } while (0)
