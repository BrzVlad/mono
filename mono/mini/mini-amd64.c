...
mono_arch_set_native_call_context (CallContext *ccontext, gpointer frame, MonoMethodSignature *sig)
{
	...
	if (sig->ret->type != MONO_TYPE_VOID) {
		if (cinfo->ret.storage == ArgValuetypeAddrInIReg) {
			gpointer ret_storage = mono_interp_frame_arg_to_storage ((MonoInterpFrameHandle)frame, sig, -1);
	...
	for (int i = 0; i < sig->param_count + sig->hasthis; i++) {
		...
		mono_interp_frame_arg_to_data ((MonoInterpFrameHandle)frame, sig, i, storage);
		if (storage_type == ArgValuetypeInReg) {
			...
			for (int k = 0; k < ainfo->nregs; k++) {
				...
				switch (storage_type) {
					...
					case ArgInFloatSSEReg:
					case ArgInDoubleSSEReg:
						ccontext->fregs [reg_storage] = *(mgreg_t*)storage;
						break;
					default:
						g_assert_not_reached ();
				}
...
mono_arch_get_native_call_context (CallContext *ccontext, gpointer frame, MonoMethodSignature *sig)
{
		...
		case ArgValuetypeInReg: {
			...
			for (int k = 0; k < ainfo->nregs; k++) {
				...
				switch (storage_type) {
					...
					case ArgInFloatSSEReg:
					case ArgInDoubleSSEReg:
						*storage_tmp = ccontext->fregs [reg_storage];
						break;
					default:
						g_assert_not_reached ();
				}
	...
	mono_interp_data_to_frame_arg ((MonoInterpFrameHandle)frame, sig, -1, storage);
}
