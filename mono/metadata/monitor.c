...
static inline guint32 get_thread_id ()
{
#ifdef HAVE_KW_THREAD
	return tls_small_id;
#else
...
static inline guint32 mon_status_get_owner (guint32 status)
{
	return status & OWNER_MASK;
}

static inline guint32 mon_status_set_owner (guint32 status, guint32 owner)
{
	return (status & ENTRY_COUNT_MASK) | owner;
}

static inline gint32 mon_status_get_entry_count (guint32 status)
{
...
static inline guint32 mon_status_init_entry_count (guint32 status)
{
	return (status & OWNER_MASK) | ENTRY_COUNT_ZERO;
}

static inline guint32 mon_status_increment_entry_count (guint32 status)
{
	return status + (1 << ENTRY_COUNT_SHIFT);
}

static inline guint32 mon_status_decrement_entry_count (guint32 status)
{
	return status - (1 << ENTRY_COUNT_SHIFT);
}

static inline gboolean mon_status_have_waiters (guint32 status)
{
...
mono_monitor_init_tls (void)
{
	...
	tls_small_id = mono_thread_info_get_small_id ();
#endif
}
...
mono_monitor_exit (MonoObject *obj)
{
	...
	old_status = mon->status;
	if (G_UNLIKELY (mon_status_get_owner (old_status) != get_thread_id ())) {
		return;
	}
	...
	if (nest == 0) {
		...
		LOCK_DEBUG (g_message ("%s: (%d) Object %p is now unlocked", __func__, get_thread_id (), obj));
		...
		LOCK_DEBUG (g_message ("%s: (%d) Object %p is now locked %d times", __func__, get_thread_id (), obj, nest));
		mon->nest = nest;
	}
}
...
ves_icall_System_Threading_Monitor_Monitor_test_owner (MonoObject *obj)
{
	...
	if (mon_status_get_owner (mon->status) == get_thread_id ()) {
		return(TRUE);
	}
