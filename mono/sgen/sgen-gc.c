...
static void
sgen_wbroot_scan_card_table (void** start_root, mword size, int element_size, char *bitmap,  ScanCopyContext ctx)
{
	ScanPtrFieldFunc scan_field_func = ctx.ops->scan_ptr_field;
...
LOOP_HEAD:
	...
	for (; card_data < card_data_end; card_data = sgen_find_next_card (card_data + 1, card_data_end)) {
		...
		for (; elem < card_end; elem += SIZEOF_VOID_P) {
			if (bitmap [((void**)elem - start_root) % element_size])
...
sgen_wbroots_scan_card_table (ScanCopyContext ctx)
{
	...
	SGEN_HASH_TABLE_FOREACH (&roots_hash [ROOT_TYPE_WBARRIER], void **, start_root, RootRecord *, root) {
		...
		SGEN_ROOT_DESC_VECTOR_READ (root->root_desc, element_size, bitmap);

		sgen_wbroot_scan_card_table (start_root, (mword)root->end_root - (mword)start_root, element_size, bitmap, ctx);
	} SGEN_HASH_TABLE_FOREACH_END;
}
