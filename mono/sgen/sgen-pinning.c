...
/* This pin queue records only objects that are pinned */
...
struct _CementHashEntry {
	GCObject *obj;
	unsigned int count;
	gboolean forced; /* if it should remain cemented after the finishing pause */
};
...
sgen_cement_force_pinned (void)
{
	...
	for (i = 0; i < SGEN_CEMENT_HASH_SIZE; i++) {
		...
		if (pin_queue.data [index] < (gpointer)((char*)obj + sgen_safe_object_get_size (obj)) &&
				pin_queue.data [index] >= (gpointer)obj)
