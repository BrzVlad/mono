...
static void
major_scan_card_table (CardTableScanType scan_type, ScanCopyContext ctx, int worker_index, int worker_count)
{
	MSBlockInfo *block;
	...
	FOREACH_BLOCK_HAS_REFERENCES_NO_LOCK (block, has_references) {
		if (__index % worker_count != worker_index)
			continue;
#ifdef PREFETCH_CARDS
		int prefetch_index = __index + 6 * worker_count;
